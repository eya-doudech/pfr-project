    ...
    <div class="card">
      ...
      <div class="table-responsive">
        ...
        <table class="table align-items-center table-flush" id="user-table">
          ...
          <tbody>
            @if(!empty($trashed_departements))
            @foreach ($trashed_departements as $key => $departement)
              <tr>
                <td scope="row">{{ $key++ }}</td>
                <td scope="row">{{ $departement->designation }}</td>
                <td scope="row">{{ $departement->localisation }}</td>
                <td scope="row"><button class="btn btn-sm btn-danger deleteCtg" data-link="{{ URL('/trashed/departements/restore/'.$departement->id) }}" data-token="{{csrf_token()}}" >Restore</button></td>
              </tr>
            @endforeach
            @endif
